...
test.describe('Home Page E2E', () => {
	...
	test.describe('Page Loading and Content', () => {
		...
		test('should display hero content correctly', async ({ page }) => {
			// Check main title
			await expect(page.locator('[data-test="hero-title"]')).toBeVisible()
			await expect(page.locator('[data-test="hero-title"]')).toContainText('MsDacia')

			// Check subtitle with pronunciation
			await expect(page.locator('[data-test="hero-subtitle"]')).toBeVisible()
			await expect(page.locator('[data-test="hero-subtitle"]')).toContainText('pronounced')
			await expect(page.locator('[data-test="hero-subtitle"] em')).toContainText('day-sha')

			// Check tagline
			await expect(page.locator('[data-test="tagline"]')).toBeVisible()
			await expect(page.locator('[data-test="tagline"]')).toContainText('I am here. This is who I am. I am Dacia.')
		})

		test('should display call-to-action buttons', async ({ page }) => {
			const ctaButtons = page.locator('[data-test="cta-button"]')
	...
	test.describe('Navigation from Home', () => {
		...
		test('should navigate to Portfolio page when "View Portfolio" is clicked', async ({ page }) => {
			await page.locator('[data-test="cta-button"]', { hasText: 'View Portfolio' }).click()
	...
	test.describe('Responsive Design', () => {
		test('should display correctly on mobile devices', async ({ page }) => {
			...
			await expect(page.locator('[data-test="hero-subtitle"]')).toBeVisible()
			await expect(page.locator('[data-test="tagline"]')).toBeVisible()
			await expect(page.locator('[data-test="quick-links"]')).toBeVisible()
		...
		test('should handle very small screens gracefully', async ({ page }) => {
			...
			await expect(page.locator('[data-test="hero-title"]')).toBeVisible()
			await expect(page.locator('[data-test="cta-button"]').first()).toBeVisible()

			// Should be able to navigate
			await page.locator('[data-test="cta-button"]').first().click()
	...
	test.describe('Typography and Visual Design', () => {
		...
		test('should emphasize pronunciation correctly', async ({ page }) => {
			const emphasis = page.locator('[data-test="hero-subtitle"] em')
			...
			const subtitleColor = await page.locator('[data-test="hero-subtitle"]').evaluate(el => getComputedStyle(el).color)
			expect(emphasisColor).not.toBe(subtitleColor)
		})

		test('should have proper button styling and hover effects', async ({ page }) => {
			const primaryBtn = page.locator('[data-test="cta-button"]').first()
			const secondaryBtn = page.locator('[data-test="cta-button"].secondary')
		...
		test('should have sufficient color contrast', async ({ page }) => {
			...
			for (const theme of themes) {
				...
				if (await themeSwitcher.isVisible()) {
					...
					if (await menuTrigger.isVisible()) {
						await menuTrigger.click()
						await page.locator('[data-test="theme-option"]', { hasText: theme === 'light' ? /^\s*Light\s*$/ : /^\s*Dark\s*$/ }).click()
					}
				}
				...
				await expect(page.locator('[data-test="cta-button"]').first()).toBeVisible()
				await expect(page.locator('[data-test="cta-button"]').last()).toBeVisible()
			}
		})
	})
	...
	test.describe('Performance and Loading', () => {
		test('should load quickly', async ({ page }) => {
			...
			await page.goto('/')
			await page.waitForSelector('[data-test="hero-title"]')
	...
	test.describe('User Experience', () => {
		test('should provide clear user journey', async ({ page }) => {
			...
			await expect(page.locator('[data-test="hero-title"]')).toContainText('MsDacia')
			await expect(page.locator('[data-test="hero-subtitle"]')).toContainText('day-sha') // Pronunciation help

			// Clear next steps should be available
			await expect(page.locator('[data-test="cta-button"]', { hasText: 'Learn About Me' })).toBeVisible()
			await expect(page.locator('[data-test="cta-button"]', { hasText: 'View Portfolio' })).toBeVisible()
		...
		test('should maintain consistency across visits', async ({ page }) => {
			...
			const titleText = await page.locator('[data-test="hero-title"]').textContent()
			const taglineText = await page.locator('[data-test="tagline"]').textContent()
			...
			await expect(page.locator('[data-test="hero-title"]')).toContainText(titleText || '')
			await expect(page.locator('[data-test="tagline"]')).toContainText(taglineText || '')
		})
	})
	...
	test.describe('Integration with Site Navigation', () => {
		test('should integrate well with header navigation', async ({ page }) => {
			...
			const menuToggle = page.locator('[data-test="menu-toggle"]') // Menu toggle in header
		...
		test('should show active state when home is current page', async ({ page }) => {
			...
			if (await menuToggle.isVisible()) {
				...
				const homeLink = page.locator('[data-test="nav-link"]', { hasText: 'Home' })
